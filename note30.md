...
```ruby=
%w[sanfrecce_osaka igaiga masuyama13 hogucc].
  partition { |user| !%w[masuyama13 sanfrecce_osaka chiroru].include?(user) }.
  flat_map(&:shuffle)
  # => ["hogucc", "igaiga", "masuyama13", "sanfrecce_osaka"]
```
...
- @hogucc 先週残っていたactive-storage-validateの問題をigaigaさんに確認できてよかった
- @masuyama13 RubyMineの色がおかしくなって焦った。バリデーションエラーはよくわからん
- @chiroru ブランチ名、なるべく細かく書けるところは書こう。 xxx-yyy2みたいなのはだめ
- @sanfrecce-osaka いよいよ皆就職が決まってきて参加者が減ってきた(´・ω・｀)
  - @hogucc (´・ω・｀)
- @sanfrecce-osaka active-storage-validate に対してはいい感じに PR を残せた